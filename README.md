...
> ⚠️ **一个例外——可选的 LLM 解析（`--use-llm`）**：默认的解析是**纯本地、不联网**的。
> 只有当你显式开启 `--use-llm` 时，工具才会把询价单原始内容（含客户名、产品、数量）
> 发送到 Anthropic 云端 API 帮忙解析。这个开关**默认关闭、需要你主动 opt-in**；
> 处理敏感或保密单据时请谨慎，或直接用默认的本地模式。
